...
class Loader():
    ...
    def count_rows_in_table_Generators_of_database_AirTable_with_existing_GATS_ID_in_column_unit_id_of_table_Generators_of_database_RECBus(self):       
        data_frame_Generators_of_database_AirTable = pd.read_json(path_or_buf = self.path_to_data + "AirTable/Generators.ndjson", lines = True)
        data_frame_Generators_of_database_RECBus = pd.read_csv(filepath_or_buffer = self.path_to_data + "RECBus/Generators.csv", header = 0)
        filtered_data_frame = data_frame_Generators_of_database_AirTable[data_frame_Generators_of_database_AirTable["GATS ID"].notnull() & data_frame_Generators_of_database_AirTable["GATS ID"].isin(data_frame_Generators_of_database_RECBus["unit-id"])]
    ...
    def count_rows_in_table_Generators_of_database_RECBus_with_existing_unit_id_in_column_GATS_ID_of_table_Generators_of_database_AirTable(self):
        data_frame_Generators_of_database_RECBus = pd.read_csv(filepath_or_buffer = self.path_to_data + "RECBus/Generators.csv", header = 0)
        data_frame_representing_table_Generators_of_database_AirTable = pd.read_json(path_or_buf = self.path_to_data + "AirTable/Generators.ndjson", lines = True)
